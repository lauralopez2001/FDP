...
#subseting the variables to be used to group the data
new_consumption <- consumption_user[,c(1, 3, 11, 30)]

#summing all KCal for each subject on every hour each different day 
...
#We are summing hour 24 to hour 0 since its the same hour but some subjects reported it as 00:30 and some as 24:30.
...
#order dataframe, we want the order to be by hour, so the hour 00 for the first day will be first followed by the 
#hour 00 of the second day etc This way it will be easier to compute the metrics
...
##CODE TO OBTAIN the amount of weekend days for each subject in weekendcode.R

#add weekend variable to the whole dataset
...
#Here we are transforming 2 variables that give information on smoking statuts to only one variable
#we are also transforming 2 variables that give information on amount of alcholto only one variable
